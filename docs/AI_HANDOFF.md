...
Files changed for 4.10.0-alpha: `api/schemas.py`, `api/service.py`, `openwebui/examples/openwebui_connection_example.md`, `docs/VERSIONING.md`, `docs/AI_HANDOFF.md`, `docs/PHASE_LOG.md`, and `README.md`. Manual validation pending.
...
Manually validate 4.10.0-alpha conversation context resolution. Confirm history-derived community resolution works when the current question is ambiguous, existing stateless callers still work with no `history` field, and DeepSeek receives only the current question.
