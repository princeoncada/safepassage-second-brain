...
Next version: 4.9.0-alpha. Next phase: Community Onboarding + Scope-Aware Retrieval Improvements. First task: ingest post orders for remaining unindexed communities via `/post-orders [ALIAS] [text]` in Open WebUI, then validate `/announcements` YES confirmation flow.
